...
const scrapLetter = async (letter: string): Promise<ElixWord[]> => {
  ...
  Logger.debug(`Found ${chalk.blue.bold(allLetterSearchLinks.length)} links for the letter ${chalk.blue.bold(letter)}.`)

  const allWordsDetailLinks = (
    await Promise.all(
      allLetterSearchLinks.map(async (searchLink) => await listAllDetailLinksFromSearchPage(searchLink))
    )
  ).flatMap((l) => l)

  // Remove duplicates
  const cleanedDetailLinks = allWordsDetailLinks.filter(
    (detailLink, index) => allWordsDetailLinks.findIndex((l) => l.link === detailLink.link) === index
  )
  Logger.debug(
    `Found ${chalk.blue.bold(cleanedDetailLinks.length)} detail links (without duplicates - of totla ${
      allWordsDetailLinks.length
    }).`
  )
  if (cleanedDetailLinks.length !== allWordsDetailLinks.length) {
    ...
    duplicateLinks.forEach((duplicate) => {
      Logger.debug(
        `\t- ${duplicate.title}: ${duplicate.link} ${duplicate.traceLink ? `(from ${duplicate.traceLink})` : ""}`
      )
    })
  }

  const words = (
    await Promise.all(cleanedDetailLinks.map(async (word) => await extractWordFromDetailPage(word)))
  ).flatMap((w) => w)
