    ...
    for (const datum of data) {
        ...
        if (!existingWords.length) {
            ...
            if (!existingWord && datum.definition) {
                ...
                stats.inserted++
                Logger.debug(
                    `There ${existingWords.length} occurence${s(existingWords.length)} of the word "${datum.word}" exist in the database but with no exact match. This word has been inserted.`,
                    result._id
                )
                continue
            }
...
const findExactMatchExistingWord = (existingWords: DBWord[], wordToTreat: Word): DBWord | null => {
    ...
    return (
        findMatchingWords(
            existingWords,
            wordToTreat,
            (existingWord) => existingWord.sources.some(existingSource => wordToTreat.sources[0] === existingSource)
        ) ||
        findMatchingWords(
            existingWords,
            wordToTreat,
